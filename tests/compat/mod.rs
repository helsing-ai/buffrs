...
mod ensure {
    ...
    macro_rules! parsable {
        ($major:literal, $minor:literal, $patch:literal) => {
            ...
            const LIB_MANIFEST: &str = include_str!(concat!(
                "../data/legacy/v",
                stringify!($major),
                "_",
                stringify!($minor),
            ...
            const LIB_LOCKFILE: &str = include_str!(concat!(
                "../data/legacy/v",
                stringify!($major),
                "_",
                stringify!($minor),
            ...
            const API_MANIFEST: &str = include_str!(concat!(
                "../data/legacy/v",
                stringify!($major),
                "_",
                stringify!($minor),
            ...
            #[tokio::test]
            async fn parse_lockfile_unified() {
                let tmp = tempfile::TempDir::new().unwrap();
                let path = tmp.path().join("Proto.lock");
                tokio::fs::write(&path, LIB_LOCKFILE).await.unwrap();
        ...
        ($major:literal, $minor:literal, $patch:literal) => {
            ...
            fn install_api_via_registry() {
                crate::with_test_registry(|url| {
                    ...
                    let manifest = API_MANIFEST.replace("$REGISTRY", url);
    ...
    pub(crate) use installable;
}

mod v0_11_0 {
    super::ensure::parsable!(0, 11, 0);
    super::ensure::installable!(0, 11, 0);
}

mod v0_12_0 {
    super::ensure::parsable!(0, 12, 0);
    // crate::ensure::installable!(0, 12, 0);
}

mod v0_12_1 {
    super::ensure::parsable!(0, 12, 1);
    // crate::ensure::installable!(0, 12, 1);
}

mod v0_12_2 {

    super::ensure::parsable!(0, 12, 2);
    // crate::ensure::installable!(0, 12, 2);
}
